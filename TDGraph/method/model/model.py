...
class DGraphConvBn(nn.Module):
    ...
    def forward(self, data):
        ...
        for t in range(self.diffusion_iterations):
            adj_t = self.diffusion_weight * torch.matmul(adj_t, adj_t) + (1 - self.diffusion_weight) * torch.eye(x.size(0)).to(x.device)
        ...
        reg_term = self.reg_lambda * reg_term
...
class InteractiveMultiHeadAttention(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        attention_scores = torch.matmul(q1, k2.transpose(-2, -1)) / torch.sqrt(torch.tensor(self.head_dim).float())
...
class TDGraph(nn.Module):
    def __init__(self, 
        ...
        self.protein_encoder = TargetRepresentation(block_num, 
                                                    vocab_protein_size, 
                                                    embedding_size)
        self.ligand_encoder = GraphDenseNet(num_input_features=22, 
    ...
    def forward(self, data):
        ...
        attn_modulex = InteractiveMultiHeadAttention(input_dim = protein_x.shape[1], num_heads=32)
        attn_moduley = InteractiveMultiHeadAttention(input_dim = ligand_x.shape[1], num_heads=32)
        x1 = attn_modulex(protein_x, ligand_x)
        x2 = attn_moduley(ligand_x, protein_x)
